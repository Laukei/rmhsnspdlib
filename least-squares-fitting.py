		...
		for e, eta in enumerate(eta_range):
			...
			for x, exponent in enumerate(exponent_range):
				...
				if residual < minima[0]:
					minima[1], minima[2], minima[3] = d, e, x
...
D_range = np.linspace(0,30,30) #from x, to y, spaced as Z
eta_range = np.linspace(0,0.01,1001)
exponent_range = np.linspace(1,2,101)
